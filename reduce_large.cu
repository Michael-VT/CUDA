...
void reduce(const float* const d_in,float & h_reduce, int h_op, const size_t N){
  ...
  int Nblock_s=Nblock; // length of intermediate array result
  Nblock=round2power(Nblock_s); // need to be power of 2

  // Nblock=64;
  // int sizeN=N*sizeof(unsigned int);
  // printf("Block %d %d %d\n",Nthread,Nblock,Nblock_s);
  ...
  // print_arr<<<1,1>>>(d_intermediate,Nblock);
